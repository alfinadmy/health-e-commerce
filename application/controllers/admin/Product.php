...
class Product extends MY_Controller {
    ...
    public function search($page = null)
    {
        ...
        $data['title']      = 'Admin product';
        $data['konten']    = $this->product->select(
            ['product.id', 'product.title AS product_title', 'product.price', 'product.is_available', 'product.image', 'category.title AS category_title']
        )
        ->join('kategor')
    ...
    {
        $data['konten'] = $this->product->where('id', $id)->first();
        ...
        if (!empty($_FILES) && $_FILES['image']['name'] !== '') {
            ...
            if ($upload) {
                if ($data['konten']->image !== '') {
                    $this->product->deleteImage($data['konten']->image);
                }
